...
app.artistData = function artistData (username, callback) {
  var params = {};
  if (callback) {
...
app.get('/artists', function getArtistsCb (req, res) {
  app.artistData(function (err, allArtists) {
    res.render('artist-page', { artists: allArtists });
  });
});
...
app.get('/artists/:username', function getArtistCb (req, res) {
  app.artistData(req.params.username, function (err, artist) {
    if (err) throw err;
    pieces.pieceData(req.params.username, function (err, artPieces) {
      if (err) throw err;
      res.render('artist-page', { artists: artist[0], pieces: artPieces });
    });
  });
});
