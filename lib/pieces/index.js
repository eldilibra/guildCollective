var express = require('express');
var app = module.exports = express();
var mongo = require('mongojs');
var db = mongo('mongodb://heroku:silver!3@arzt.mongohq.com:10063/app13872867');
var pieces = db.collection('artists');

app.set('views', __dirname);
app.set('view engine', 'ejs');
app.use(express.bodyParser());
app.use(express.methodOverride());

app.configure(function configure (){
  app.use(express.bodyParser());
  app.use(express.methodOverride());
  app.use(app.router);
});

app.piecesData = function piecesData (username, pieceTitle, callback) {
  var params = {
    username: username
  };
  // handle 2 parameter call
  if (!callback) {
    callback = pieceTitle;
    pieceTitle = null;
  }

  if (pieceTitle) {
    params.pieceTitle = pieceTitle;
  }

  pieces.find(params, function findPieceCb (err, data) {
    if (err) throw err;
    if (data.length == 0) {
      callback(null, 'No pieces found');
      return;
    }
    callback(null, data);
  });
};

app.get('/pieces', function getArtistsCb (req, res) {
  res.write('Ecco e un elenco di opere d\'arte');
  res.end();
});

app.get('/pieces/create/:username', function createPieceCb (req, res) {
  res.render('create-artist', { username: req.params.username });
});

app.get('/pieces/:username', function getPiecesCb (req, res) {
  app.piecesData(req.params.username, function (err, pieces) {
    if (err) throw err;
    res.render('pieces-page', { pieces: pieces });
  });
});

app.get('/pieces/:username/:title', function getPieceCb (req, res) {
  app.piecesData(req.params.username, req.params.title, function (err, piece) {
    if (err) throw err;
    res.render('pieces-page', { pieces: piece });
  });
});

app.post('/pieces/:username/:title', function postPieceCb (req, res) {
  var piece = {
    username: req.params.username,
    title: req.body.title,
    piecePic: req.body.piecePic,
    creationDate: req.body.creationData,
    medium: req.body.medium,
    description: req.body.description
  };
  pieces.save(artist, function saveArtistCb(err, result) {
    if (err) throw err;
    res.redirect('/home/' + req.params.username);
  });
});
