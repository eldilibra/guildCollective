...
app.otherPiecesData = function otherPiecesData (username, pieceTitleSlug, callback) {
  if (!callback) throw new Error('Missing parameters');
  var params = {
    username: username,
    titleSlug: { $ne: pieceTitleSlug }
  };
  pieces.find(params, function findPieceCb (err, data) {
    if (err) throw err;
    if (data.length == 0) {
...
app.get('/pieces/:username/:titleSlug', function getPieceCb (req, res) {
  app.piecesData(req.params.username, req.params.titleSlug, function (err, piece) {
    if (err) throw err;
    app.otherPiecesData(req.params.username, req.params.titleSlug, function (secondErr, otherPieces) {
      if (secondErr) throw secondErr;
      var getArtist = require('../artists').artistsData;
      getArtist(req.params.username, function (thirdErr, artist) {
        if (thirdErr) throw thirdErr;
        res.render('piece-page', { artist: artist[0], piece: piece[0], otherPieces: otherPieces });
      });
    });
  });
});
